'''
data cleaning methods from NYT
'''
...
def translate_row(row):
    ...
    display_input = NLPClean.cleanUnicodeFractions(row['input'])
    ...
    for i, (token, tags) in enumerate(label_data):
        features = NLPClean.getFeatures(token, i + 1, tokens)
        translated += NLPClean.joinLine([token] + features +
                                     [_bestTag(tags)]) + '\n'
    return translated
...
def _matchUp(token, labels):
    ...
    token = NLPClean.normalizeToken(token)
    ...
    for label_key in ['name', 'unit', 'qty', 'comment', 'range_end']:
        ...
        if isinstance(label_value, str):
            for n, vt in enumerate(tokenizer.tokenize(label_value)):
                if NLPClean.normalizeToken(vt) == token:
